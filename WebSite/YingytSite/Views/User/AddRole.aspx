                    ...
                    <table id="listTable" class="table table-bordered table-hover dataTable table-striped" style="clear:right; width:100%; margin:0px; padding:0px;">
                        <tbody>
                            <tr>
                                <td rowspan="6" style="width:200px; text-align:center;">
									<label>
                                        ...
                                        ViewData["role"].ToString().Split(',').Contains("Agent") && 
                                        ViewData["role"].ToString().Split(',').Contains("Lobby") && 
                                        ViewData["role"].ToString().Split(',').Contains("User") && 
                                        ViewData["role"].ToString().Split(',').Contains("SysNotice")
                                        ) { %> checked <% } %> />
							...
							<tr>
							    <td>
									<label>
										<input name="configuration" id="configuration1" type="checkbox" class="ace indbtn li-1" data-id="1" value="Agent"
                                        <% if (ViewData["role"] != null && ViewData["role"].ToString().Split(',').Contains("Agent")) { %> checked <% } %> />
										<span class="lbl"> 代理管理</span>
									</label>
							    </td>
				            </tr>
							<tr>
							    <td>
									<label>
										<input name="configuration" id="configuration2" type="checkbox" class="ace indbtn li-1" data-id="1" value="Lobby"
                                        <% if (ViewData["role"] != null && ViewData["role"].ToString().Split(',').Contains("Lobby")) { %> checked <% } %> />
										<span class="lbl"> 营业厅管理</span>
									</label>
								</td>
				            </tr>
							<tr>
							    <td>
									<label>
										<input name="configuration" id="configuration3" type="checkbox" class="ace indbtn li-1" data-id="1" value="User"
							...
							<tr>
							    <td>
									<label>
										<input name="configuration" id="configuration4" type="checkbox" class="ace indbtn li-1" data-id="1" value="SysNotice"
                                        <% if (ViewData["role"] != null && ViewData["role"].ToString().Split(',').Contains("SysNotice")) { %> checked <% } %> />
										<span class="lbl"> 系统通知</span>
									</label>
								</td>
				            </tr>			
	...
	<script type="text/javascript">
	    ...
	    function checkRoleName() {
	        ...
	        $.ajax({
	            async: false,
	            type: "GET",
	            url: rootUri + "User/CheckUniqueRolename",
	            dataType: "json",
	            data: {
	                rid: roleid,
	                rolename: rolename
