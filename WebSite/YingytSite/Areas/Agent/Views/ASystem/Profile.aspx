					...
					<div class="tabbable">
						...
						<div class="tab-content profile-edit-tab-content">
							<div id="edit-basic" class="tab-pane in active">
								...
								<div class="row">
									<div class="col-xs-12 col-sm-4">                                        
                                        <img src="<% if ((userinfo != null)&&(userinfo.img != null)) { %><%=ViewData["rootUri"] %><%= userinfo.img %> <% } else {%><%=ViewData["rootUri"] %>Content/img/default-image_100.gif <% } %>" id = "picture" height="200px"/>	  
							...
							<div id="edit-settings" class="tab-pane">
								...
								<div>
									<label class="inline">
										<input type="checkbox" name="allowshare" id="allowshare" class="ace" <% if (userinfo != null && userinfo.allowshare == 1) { %> checked<% } %> onchange="changeShare(this)" />
										<span class="lbl"> 共有手机资料为下方的营业厅</span>
									</label>&nbsp;&nbsp;&nbsp;&nbsp;
						            <select class="select2" id="lobbylist" name="lobby_id" data-placeholder="Click to Choose...">
                                        <% 
                                            ...
                                            if (lobbylist != null)
                                            {
                                                for (int i = 0; i < lobbylist.Count(); i++)
                                                {
                                                    ...
                                                    %>
                                                    <option value="<% =item.uid %>" <% if(userinfo != null && userinfo.share_id==item.uid) { %>selected<% } %> ><% =item.username %></option>
                                                    <%
                                                }
                                            }
                                        %>
...
//	        }
        ...
        jQuery(function ($) {
	        ...
	        $('.loading-btn')
              ...
              <% if (ViewData["userinfo"] != null && ((tbl_user)ViewData["userinfo"]).allowshare==1) { %>
                    $(".select2").show();
              <% } else { %>
                    $(".select2").hide();
              <% } %>
            ...
            function submitform() {
				$.ajax({
				    type: "POST",
				    url: rootUri + "Agent/ASystem/SubmitUserInfo",
				    dataType: "json",
