...
#define IS_FILE_EXIST(s)                        [FILE_MANAGER fileExistsAtPath : s]
...
#define roundRect(v, radio)						v.layer.cornerRadius = radio; \
												v.clipsToBounds = YES
#define borderWidthColor(v, width, color)		v.layer.borderWidth = width; \
...
// Show alert
#define alert(title,body,buttontitle)               [[[[UIAlertView alloc] initWithTitle:title message:body delegate:nil cancelButtonTitle:buttontitle otherButtonTitles:nil]] show]
#define showToast(msg)						[self.view makeToast:msg duration:DEF_DELAY position:@"center"];
